...
async function listUsers(){
  const {rows}=await pool.query(`SELECT id::text,display_name AS "顯示名稱",is_superuser AS "管理者",created_at AS "建立時間"
...
async function updateRole(nextValue){
  ...
  try{
    ...
    if(target.is_virtual||target.is_simulated)throw new Error('公費或模擬帳號不能設為管理者');
    if(!nextValue&&target.is_superuser){
      ...
      if(count.total<=1)throw new Error('系統至少需要保留一位管理者');
    }
    await client.query('UPDATE users SET is_superuser=$1,updated_at=now() WHERE id=$2',[nextValue,userId]);
    await client.query('COMMIT');
    console.log(`已${nextValue?'授予':'移除'}「${target.display_name}」的管理者權限`);
  }catch(error){
...
try{
  ...
  console.error(`管理者設定失敗：${error.message}`);
  process.exitCode=1;
}finally{
  await pool.end();
}
