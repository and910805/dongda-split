...
export default function ProductApp({Home}){
  ...
  return <div className="real-app">
    ...
    <section className="real-workspace">
      ...
      {!groups.length?<EmptyGroups create={()=>setShowCreate(true)}/>:groupError&&!group?<WorkspaceError message={groupError} retry={refreshGroup}/>:!group?<DashboardSkeleton/>:<GroupDashboard group={group} me={me} addExpense={openNewExpense} editExpense={expense=>{setEditingExpense(expense);setShowExpense(true)}} invite={()=>setShowInvite(true)} removeGroup={()=>groupDeleted(group)} refresh={refreshGroup} refreshing={groupLoading}/>}
    </section>
  ...
  return <main className={`real-dashboard ${refreshing?'is-refreshing':''}`} aria-busy={refreshing}>
    ...
    <section className="group-hero" aria-labelledby="group-title">
      ...
      <div className="group-overview-side">
        {group.ownerId===me.id&&<details className="group-admin-menu"><summary><Settings2/> 群組設定</summary><div><p>刪除後，所有支出與結算都無法復原</p><button className="danger-action" disabled={deletingGroup} onClick={deleteCurrentGroup}>{deletingGroup?<LoaderCircle/>:<Trash2/>}{deletingGroup?'刪除中…':'刪除群組'}</button></div></details>}
      </div>
    </section>
    ...
    <div className="real-grid">
        ...
        <section className="expense-panel" aria-labelledby="expense-title">
          ...
          {!group.expenses.length?<div className="empty-list"><ReceiptText/><b>還沒有任何支出</b><p>從第一筆共同花費開始建立清楚帳目</p><button className="empty-primary" onClick={addExpense}><Plus/> 新增第一筆支出</button></div>:<>
            ...
            <div className="record-list">{group.expenses.map(e=><article key={e.id}>
              ...
              <div className="record-price"><b className={e.amountCents<0?'positive':''}>{money(e.amountCents)}</b><small>{e.payerCount>1?`${e.payerCount} 人付款`:`平均 ${money(Math.round(e.amountCents/e.shareCount))}`}</small></div>
      ...
      <aside className="settlements" aria-labelledby="settlement-title">
        ...
        {!group.settlements.length?<div className="all-clear"><Check/><b>目前都結清了</b><p>新增支出後，旅帳會在這裡整理最少轉帳路徑</p></div>:<div className="settlement-list">{group.settlements.map((s,i)=><article className={`settlement-row ${canConfirm(s)?'payable':''}`} key={i}><span className={`settlement-status ${canConfirm(s)?'needs-action':''}`}>{canConfirm(s)?'待你處理':'等待對方'}</span><div className="settlement-person"><Person person={s.from} size={44}/><span className="settlement-person-copy"><small>付款人</small><b>{s.from.displayName}</b></span></div><div className="settlement-flow"><ArrowRight/><small>應付金額</small><strong>{money(s.amountCents)}</strong></div><div className="settlement-person receiver"><Person person={s.to} size={44}/><span className="settlement-person-copy"><small>收款人</small><b>{s.to.displayName}</b></span></div>{canConfirm(s)?<button disabled={Boolean(paying)} onClick={()=>markPaid(s)}>{paying===s.to.id?<><LoaderCircle/>處理中…</>:canManageFundPayment(s)?'從公費付款':'我已轉帳'}</button>:<div className="settlement-waiting">等待付款人轉帳</div>}</article>)}</div>}
      </aside>
      <SettlementHistory group={group}/>
...
function SettlementHistory({group}){
 ...
 return <section className="repayment-panel" aria-labelledby="repayment-title">
  <div className="section-head"><div><h2 id="repayment-title">還款紀錄</h2><p>已完成的實際轉帳，共 {rows.length} 筆</p></div><button type="button" className="history-refresh" aria-label="還款紀錄已是最新狀態"><RefreshCcw/></button></div>
...
function BalanceChart({group,close}){
 ...
 return <Modal close={close} label="群組結餘"><span className="eyebrow"><WalletCards/> Balance</span><h2>群組結餘</h2><p className="modal-copy">每個人的最終淨額：紅色代表需要付出，綠色代表可以收回長度越長，金額越大</p><div className="diverging-legend"><span>← 應付</span><i></i><span>應收 →</span></div><div className="diverging-chart">{rows.map(person=>{const value=person.balanceCents,width=value===0?0:Math.max(10,Math.round(Math.abs(value)/maximum*100)),member=<div className="diverging-member"><b>{person.displayName}</b><Person person={person} size={38}/></div>;return <div className={`diverging-row ${value<0?'is-debt':value>0?'is-credit':'is-zero'}`} key={person.id}><div className="diverging-left">{value<0?<span className="diverging-bar debt" style={{width:`${width}%`}}><strong>-{money(Math.abs(value))}</strong></span>:member}</div><div className="diverging-right">{value>0?<span className="diverging-bar credit" style={{width:`${width}%`}}><strong>+{money(value)}</strong></span>:value<0?member:<strong className="zero-amount">{money(0)}</strong>}</div></div>})}</div><div className="balance-check"><Check/> 所有人的結餘加總為 NT$ 0</div><button className="primary wide" onClick={close}>了解</button></Modal>
}
function SettlementHelp({close}){return <Modal close={close} label="結算說明"><span className="eyebrow"><CircleHelp/> 結算說明</span><h2>我們如何簡化轉帳？</h2><p className="modal-copy">系統會先把所有支出換算成每個人的最終淨額，再重新安排付款對象原始帳目不會被修改，每個人最後付出或收到的總額也完全相同</p><div className="settlement-example"><div className="example-title"><b>舉個例子</b><small>A、B、C 三人結算</small></div><div className="example-columns"><section><b>原始 · 3 筆</b><p>A → C　NT$ 100</p><p>B → C　NT$ 300</p><p>A → B　NT$ 50</p></section><ArrowRight/><section className="simplified"><b>簡化後 · 2 筆</b><p>A → C　NT$ 150</p><p>B → C　NT$ 250</p></section></div><div className="example-net"><span>A 應付 150</span><span>B 應付 250</span><span>C 應收 400</span></div></div><div className="algorithm-note"><b>目前採用的方式</b><p>先配對金額完全相同的人，再讓小額欠款者優先一次付清目標是讓更多付款人只轉一次；較大的欠款者必要時可能拆成多筆，因此不保證全群總筆數是數學上的絕對最少</p></div><button className="primary wide" onClick={close}>了解</button></Modal>}
function Modal({children,close,label='對話視窗'}){const dialogRef=useRef(null),closeRef=useRef(close),returnFocus=useRef(document.activeElement);closeRef.current=close;useEffect(()=>{const dialog=dialogRef.current,focusable='button:not([disabled]), a[href], input:not([disabled]), select:not([disabled]), textarea:not([disabled]), [tabindex]:not([tabindex="-1"])';const initialTimer=setTimeout(()=>{if(dialog&&!dialog.contains(document.activeElement))dialog.querySelector(focusable)?.focus()},0);const onKeyDown=event=>{if(event.key==='Escape'){event.preventDefault();closeRef.current();return}if(event.key!=='Tab'||!dialog)return;const items=[...dialog.querySelectorAll(focusable)].filter(item=>item.getClientRects().length);if(!items.length)return;const first=items[0],last=items.at(-1);if(event.shiftKey&&document.activeElement===first){event.preventDefault();last.focus()}else if(!event.shiftKey&&document.activeElement===last){event.preventDefault();first.focus()}};document.addEventListener('keydown',onKeyDown);return()=>{clearTimeout(initialTimer);document.removeEventListener('keydown',onKeyDown);returnFocus.current?.focus?.()}},[]);return <div className="overlay" onMouseDown={e=>e.target===e.currentTarget&&close()}><div ref={dialogRef} className="modal real-modal" role="dialog" aria-modal="true" aria-label={label}><button type="button" className="modal-x" onClick={close} aria-label="關閉對話視窗"><X/></button>{children}</div></div>}
