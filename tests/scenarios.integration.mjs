...
try{
  ...
  const fundGroup=await post(owner.cookie,'/api/groups',{name:'fund-e2e',description:'scenario automated test'});for(const actor of actors.slice(1))await post(actor.cookie,`/api/invites/${fundGroup.inviteToken}/join`,{});const fund=await post(owner.cookie,`/api/groups/${fundGroup.id}/funds`,{name:'公費'});for(const actor of actors)await post(owner.cookie,`/api/groups/${fundGroup.id}/funds/${fund.id}/contributions`,{fromUserId:actor.user.id,amount:1000});await post(owner.cookie,`/api/groups/${fundGroup.id}/expenses`,{title:'公費買門票',amount:2800,payerId:fund.id,participantIds:ids,splitMode:'equal'});const {data:fundDetail}=await request(`/api/groups/${fundGroup.id}`,{cookie:owner.cookie});const fundBalance=fundDetail.balances.find(x=>x.id===fund.id);assert.equal(fundBalance.balanceCents,-1_120_000);assert.ok(fundDetail.balances.filter(x=>!x.isFund).every(x=>x.balanceCents===80_000));assert.equal(fundDetail.balances.reduce((sum,x)=>sum+x.balanceCents,0),0);
